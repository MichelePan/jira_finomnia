...
def render_kpis(df: pd.DataFrame):
    ...
    unassigned_tasks = int((df["Assignee"].fillna("").str.strip() == "").sum())

    stale_tasks = int(
        (
            (df["Done"] == False)
            & (df["DaysSinceUpdate"].fillna(0) >= st.session_state.get("stale_days", 7))
        ).sum()
    )
    ...
    c6.metric("Non assegnati", unassigned_tasks)
    c7.metric("Epic", df["EpicKey"].replace("", pd.NA).dropna().nunique())
    c8.metric("Assignee", df["Assignee"].replace("", pd.NA).dropna().nunique())
    c9.metric("Issue Type", df["IssueType"].replace("", pd.NA).dropna().nunique())
    c10.metric("Priorità", df["Priority"].replace("", pd.NA).dropna().nunique())

def render_status_panel(df: pd.DataFrame):
...
def render_status_category_panel(df: pd.DataFrame):
    st.subheader("Task per categoria Jira")
...
def render_epic_panel(df: pd.DataFrame):
    ...
    grouped["Avanzamento %"] = (grouped["Completati"] / grouped["Task"] * 100).round(1)
...
def render_assignee_panel(df: pd.DataFrame):
    ...
    assignee_df["Assignee"] = assignee_df["Assignee"].fillna("").replace("", "Non assegnato")
...
def render_priority_panel(df: pd.DataFrame):
    ...
    priority_df = df.copy()
    priority_df["Priority"] = priority_df["Priority"].fillna("").replace("", "Nessuna priorità")
...
def render_age_panel(df: pd.DataFrame):
    ...
    existing_columns = [column for column in columns if column in open_df.columns]
...
def render_alerts_panel(df: pd.DataFrame, stale_days: int):
    ...
    with tab1:
        render_alert_table(stale_df, columns)

    with tab2:
        render_alert_table(unassigned_df, columns)

    with tab3:
        render_alert_table(no_epic_df, columns)

    with tab4:
        render_alert_table(overdue_df, columns)

def render_alert_table(df: pd.DataFrame, columns: list[str]):
    if df.empty:
...
def render_detail_table(df: pd.DataFrame):
    ...
    existing_columns = [column for column in columns if column in detail_df.columns]
